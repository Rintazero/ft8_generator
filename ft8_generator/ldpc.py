...
FT8_LDPC_N = 174
FT8_LDPC_K = 91
FT8_LDPC_M = FT8_LDPC_N - FT8_LDPC_K
FT8_LDPC_N_BYTES = (FT8_LDPC_N + 7) // 8
FT8_LDPC_K_BYTES = (FT8_LDPC_K + 7) // 8
...
# 奇偶校验 return 1 if odd number of '1' s, otherwise 0
def parity_check_8bits(x: np.uint8) -> bool:
    # 将所有的 位 同最低位进行异或, 最终取出最低位 即可
    x ^= x >> 4
    x ^= x >> 2
    x ^= x >> 1
    return x % 2

def ldpc_generator(a91_12bytes: NDArray[np.uint8]) -> NDArray[np.uint8]:

    codeword = np.zeros(FT8_LDPC_N_BYTES, dtype=np.uint8)

    for i in range(FT8_LDPC_K_BYTES):
        codeword[i] = a91_12bytes[i]
    ...
    col_mask = np.uint8(0x80 >> (FT8_LDPC_K % 8))
    col_idx = np.uint8(FT8_LDPC_K_BYTES - 1)

    for i in range(FT8_LDPC_M):
        nsum = np.uint8(0)
        for j in range(FT8_LDPC_K_BYTES):
            bits = np.uint8( FT8_LDPC_G[i][j] & a91_12bytes[j] ) # 矩阵乘法, G[i][j] & msg[j]
            nsum ^= parity_check_8bits(bits) # 相乘后求和
        if nsum != 0:
